...
• بررسی امنیت و چک‌لیست در سطح Test Request، فقط پس از انتخاب صریح سرپرست QA، با Template مدیریتی و Revision.
...
مشاهده درخواست‌های دارای الزام امنیت، شواهد تست و اطلاعات محیط؛ تکمیل اجباری Checklist همان درخواست، Attachment و Revision تا قبل از تصمیم نهایی. Security Reviewer
...
۴-۱۰. بررسی امنیتی در سطح Test Request
• بررسی امنیت فقط وقتی ایجاد می‌شود که QA Lead هنگام ثبت وضعیت «آماده» یا «آماده مشروط»، گزینه «احتیاج به تست امنیت دارد» را انتخاب کند.
• بدون انتخاب این گزینه، درخواست نباید در کارتابل Security Reviewer نمایش داده شود.
• برای هر Test Request یک SecurityReview ایجاد می‌شود، نه برای هر Test Case.
• سرپرست QA پیش از ارجاع باید اطلاعات فنی سامانه، دسترسی، محیط و احراز هویت متناسب با Development/Test/Production را کامل کند. رمز عبور در فرم ثبت نمی‌شود و فقط روش تحویل امن آن نگهداری می‌شود.
• تمام ورودی‌های تاریخ و تاریخ‌وساعت این فرم شمسی هستند.
• کارتابل امنیت خلاصه درخواست، افراد مسئول، نسخه/Build و شمارنده‌های قابل Drill-down برای Test Case، Run و خطاهای باز Blocker/Critical را نمایش می‌دهد.
• همه آیتم‌های Checklist باید نتیجه PASS/FAIL/PARTIAL/N_A داشته باشند؛ گزینه «تست نشده» در این گردش‌کار وجود ندارد.
• برای تکمیل Checklist، بارگذاری حداقل یک مستند امنیت اجباری است و سقف هر فایل ۱۰ مگابایت است.
• رکورد تکمیل‌شده از کارتابل حذف نمی‌شود و فقط وضعیت آن تغییر می‌کند.
• اگر همه نتایج PASS یا N_A باشند، VersionHistory برای تصمیم نسخه‌گذاری نهایی به سرپرست فنی می‌رود.
• وجود FAIL یا PARTIAL پرونده را همراه چک‌لیست و مستندات به کارتابل «بررسی امنیت» سرپرست QA می‌فرستد.
• سرپرست QA با توضیح اجباری می‌تواند پرونده را به متخصص QA ارجاع دهد یا برای اصلاح به تیم امنیت برگرداند.
• متخصص QA موارد را در قالب «اجرای امنیتی» به برنامه‌نویس ارجاع می‌دهد؛ پس از ثبت رفع، گزارش حداکثر ۱۰ مگابایتی را برای سرپرست QA می‌فرستد.
• تأیید گزارش توسط سرپرست QA پرونده را برای بازبینی مجدد به تیم امنیت می‌فرستد؛ رد گزارش آن را به متخصص QA بازمی‌گرداند.
• همه رفت‌وبرگشت‌ها با نقش و کاربر اقدام‌کننده، زمان، توضیح، وضعیت مبدأ/مقصد و فایل‌های مرتبط قابل ردیابی هستند.
...
• تکمیل بررسی، VersionHistory را به وضعیت منتظر تصمیم می‌برد و برای سرپرست فنی اعلان ایجاد می‌کند.

شکل ۱۴ - جریان بازبینی امنیت
قالب چک‌لیست از Back-office مدیریت می‌شود و Review درخواست تست در Snapshot انتشار ثبت می‌شود.
...
به اجرای نهایی دیگری نیاز است و دلیل آن باید برای متخصص QA ثبت شود. احتیاج به اجرای مجدد

در «احتیاج به اجرای مجدد»، توضیح اجباری است، درخواست در حال انجام می‌ماند و همراه دلیل و اعلان به متخصص QA برمی‌گردد. پس از ثبت اجرای نهایی جدید، کارت از فهرست اجرای مجدد خارج و برای بررسی دوباره به QA Lead ارجاع می‌شود. اگر Build در درخواست خالی باشد، QA Lead می‌تواند آن را در همین مرحله تکمیل کند. گزینه نیاز به تست امنیت فقط برای «آماده انتشار» و «آماده انتشار مشروط» قابل انتخاب است؛ در صورت انتخاب، اطلاعات امنیت اجباری و مرحله بعد `SECURITY_REVIEW` است و در غیر این صورت درخواست وارد کارتابل امنیت نمی‌شود. Modal وضعیت کیفیت باید جزئیات کامل درخواست تست و داده‌های ثبت‌شده در فرم درخواست را نیز در اختیار کاربر قرار دهد.
...
بازبینی امنیتی هر Test Request و Revisionهای آن. SecurityReview/Revision
...
ایجاد SecurityReview فقط با انتخاب QA Lead و به‌ازای هر Test Request. FR-034
تکمیل اجباری همه آیتم‌ها، مستند حداکثر ۱۰ مگابایتی و مسیریابی نتیجه براساس PASS/N_A یا FAIL/PARTIAL. FR-035
...
اعلام نظر QA با چهار وضعیت فارسی و امکان درخواست اجرای مجدد با دلیل. FR-038
...
اعلان درون‌سامانه‌ای ارجاع درخواست همراه ثبت نام گیرنده در تاریخچه. FR-068
امکان تکمیل Build خالی توسط QA Lead هنگام ثبت وضعیت کیفیت. FR-069
اطلاعات فنی، دسترسی و احراز هویت امنیت با کنترل‌های انتخابی و تاریخ شمسی. FR-070
کارتابل بررسی امنیت برای QA Lead، QA Specialist، Developer، Security Reviewer و System Admin با اجرای امنیتی و تاریخچه رفت‌وبرگشت. FR-071
...
SecurityReview فقط برای درخواست انتخاب‌شده توسط QA Lead و به‌ازای Test Request ایجاد می‌شود. BR-021
بدون تکمیل نتیجه همه آیتم‌ها و بارگذاری مستند امنیت، Review قابل ارسال نیست؛ گزینه NOT_TESTED مجاز نیست. BR-022
فقط Review تماماً PASS/N_A به سرپرست فنی می‌رود؛ FAIL/PARTIAL باید چرخه رفع و بازبینی امنیت را طی کند. BR-022-1
...
Security Reviewer در Scope خود فقط Test Requestهای دارای الزام امنیت را می‌بیند و جزئیات تست‌کیس‌ها و اجراها را از همان درخواست Drill-down می‌کند. AC-11
Review تکمیل‌شده در فهرست باقی می‌ماند؛ فایل بزرگ‌تر از ۱۰ مگابایت رد می‌شود و تمام انتقال‌های چرخه رفع در تاریخچه دیده می‌شوند. AC-11-1
...
بدون تصمیم انتشار و ویرایش تست. درخواست‌های دارای الزام امنیت، شواهد مرتبط و Checklist همان Request در Scope. Security Reviewer
