فقط فرانت این پروژه رو به صورت کامل بزن 
...
• یک کاربر می‌تواند چند Assignment داشته باشد، اما در هر Session فقط یک Assignment/Role فعال می‌شود.
...
۳-۳. Active Context
Active Context ساختار فنی زیر را دارد: Role + ScopeType + Scope + AssignmentId. ScopeType برابر APP یا SYSTEMS است. APP به معنی دسترسی به همه Applicationهاست. SYSTEMS شامل یک یا چند ApplicationId مجاز است. Claims مربوط به Context در Token قرار می‌گیرد و در هر API، Query، Report، Download و عملیات Mutation enforce می‌شود.

شکل ۳ - مدل RBAC چندسامانه‌ای و Active Context
این مدل از Union شدن نقش‌ها جلوگیری می‌کند و هم‌زمان دو نوع دامنه کل اپ و چندسامانه‌ای را پوشش می‌دهد.

شکل ۴ - جریان ورود و انتخاب Context
پس از احراز هویت، Assignmentهای فعال بارگذاری می‌شوند و کاربر یک Context را انتخاب می‌کند؛ منو و مجوز API بر همان اساس ساخته می‌شود.
۴. الزامات عملکردی و ماژول‌ها
